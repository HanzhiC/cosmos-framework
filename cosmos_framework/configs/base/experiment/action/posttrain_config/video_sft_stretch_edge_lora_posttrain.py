...
"""``video_sft_stretch_edge_lora_posttrain`` — Cosmos3-Edge + LoRA video-only Stretch post-training.

Single-GPU-friendly variant of ``video_sft_stretch_posttrain``, aimed at fitting a
...
everything else (including the full Edge backbone) stays frozen, so FusedAdam's
fp32 master-weight/momentum/variance state is only paid for the adapters, not
the whole model.

Unlike ``video_sft_stretch_posttrain`` (which inherits Nano/FD's heavier
720p / 74000-token / ``activation_checkpointing.mode="selective"`` settings),
this recipe keeps Edge's OWN lighter defaults (480p, 45056 tokens,
``mode="full"`` checkpointing) rather than the Stretch-FD-inherited values —
deliberately, to minimize memory footprint. The only deltas from
``EDGE_MODEL_CONFIG`` are: LoRA enablement, ``diffusion_expert_config.
load_weights_from_pretrained=False`` (load fully from the converted Cosmos3-Edge
DCP checkpoint, not a separate HF pretrain fetch), and
``tokenizer.encode_exact_durations=[17]`` (matches the dataset's fixed
chunk_length=16 -> 17-frame window, independent of resolution).

Same ``mask_action=True`` / video-only caveats as ``video_sft_stretch_posttrain``
apply here (see that module's docstring) — this is I2V-style generation with a
masked/zeroed action, not multi-frame V2V prefix continuation.

For single-GPU use, set ``model.parallelism.data_parallel_shard_degree=1`` and
``NPROC_PER_NODE=1`` at launch (see the TOML / launch shell) — this recipe is
otherwise architecturally the same as the multi-GPU Stretch recipes.

Unvalidated in this repo: neither Cosmos3-Edge nor LoRA has been run on Stretch
data before, and no single-GPU memory footprint has been measured here for any
recipe. Treat this as a starting point for an empirical 48GB smoke test, not a
known-good config.
"""
...
video_sft_stretch_edge_lora_posttrain = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            ...
            group="sft",
            name="${now:%Y-%m-%d_%H-%M-%S}_video_sft_stretch_edge_lora_posttrain",
            wandb_mode="disabled",
        ),
        model=dict(
        ...
        optimizer=dict(
            ...
            eps=1.0e-08,
            fused=True,
            # LoRA-only: matches the prefixes injected by the LoRA adapter wiring
            # (see vision_sft_super.py). Nothing else trains.
        ...
        checkpoint=dict(
            ...
            # tensors don't exist in the base checkpoint; init fresh, matching
            # vision_sft_super's LoRA recipe).
        ...
        dataloader_train=L(PackingDataLoader)(
            ...
            dataset_name="video_stretch_edge_lora",
...
cs.store(
    group="experiment",
    package="_global_",
    name="video_sft_stretch_edge_lora_posttrain",
    node=video_sft_stretch_edge_lora_posttrain,
)
