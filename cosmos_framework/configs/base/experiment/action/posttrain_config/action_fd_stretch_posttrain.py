...
"""``action_fd_stretch_posttrain`` — Stretch+LeRobot forward-dynamics post-training.

Mirrors ``action_fd_droid_posttrain``, swapping the dataset factory for
``get_action_stretch_fd_sft_dataset`` (``StretchLeRobotDataset``: 10D ``ee_pose``
action derived from absolute end-effector pose, concat_view head+gripper camera).
``DATASET_PATH`` must point at a LeRobot root produced by
``cosmos_framework.scripts.convert_stretch_to_lerobot`` (no upstream pre-converted
Stretch dataset exists, unlike DROID/LIBERO).
"""
...
action_fd_stretch_posttrain = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            ...
            group="action_sft",
            name="${now:%Y-%m-%d_%H-%M-%S}_action_fd_stretch_posttrain",
            wandb_mode="disabled",
        ),
        model=dict(
...
cs.store(
    group="experiment",
    package="_global_",
    name="action_fd_stretch_posttrain",
    node=action_fd_stretch_posttrain,
)
