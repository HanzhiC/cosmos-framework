...
"""``action_wam_stretch_posttrain`` — Stretch+LeRobot world-action-model (WAM) post-training.

Mirrors ``action_fd_stretch_posttrain`` (same dataset/model/dataloader shape:
``StretchLeRobotDataset`` 10D ``ee_pose`` action, concat_view head+gripper camera,
chunk_length=16 @720p, token-packed ``PackingDataLoader``), swapping the dataset
``mode`` from ``"forward_dynamics"`` to ``"wam"``: given only the first video
frame (no clean actions), the model jointly denoises the future video AND the
matching future actions — the "world-action-model" framing used to train a
policy that also reasons about future visual dynamics as an auxiliary signal.
This differs from ``"inverse_dynamics"`` (predict actions from a FULLY given
video) and from ``action_fd_stretch_posttrain``'s ``"forward_dynamics"``
(predict video from FULLY given actions).

Hyperparameters here are inherited unchanged from the validated
``action_fd_stretch_posttrain`` recipe as a starting point (same data/model, only
the training objective differs) — they are **not** independently tuned for WAM;
watch the action loss on your first smoke runs and adjust ``lr`` if needed.
"""
...
action_wam_stretch_posttrain = LazyDict(
    dict(
        defaults=[
        ...
        job=dict(
            ...
            group="action_sft",
            name="${now:%Y-%m-%d_%H-%M-%S}_action_wam_stretch_posttrain",
            wandb_mode="disabled",
        ),
        model=dict(
        ...
        dataloader_train=L(PackingDataLoader)(
            ...
            dataset_name="action_stretch_wam",
...
cs.store(
    group="experiment",
    package="_global_",
    name="action_wam_stretch_posttrain",
    node=action_wam_stretch_posttrain,
)
